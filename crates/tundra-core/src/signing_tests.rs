...
fn public_multisig() -> (Psbt, Psbt, Vec<TrustedInput>) {
...
fn hwi() -> (Psbt, Psbt, Vec<TrustedInput>) {
    let signed = parse_response(include_bytes!(
...
fn ledger() -> (Psbt, Psbt, Vec<TrustedInput>) {
    let approved = parse_response(include_bytes!(
        "../../../tests/fixtures/ledger-wpkh-two-inputs.psbt"
    ))
    .unwrap();
    let data: serde_json::Value = serde_json::from_str(include_str!(
