...
fn main() -> Result<(),Box<dyn std::error::Error>> {
    let path=env::args().nth(1).ok_or("usage: cargo run -p tundra-core --example inspect -- <public-test-descriptor-file>")?;
    let bytes=fs::read(path)?;
    if bytes.len()>32_768 {return Err("descriptor file too large".into());}
    ...
    println!("Imported {:?}; receive indexes {} and {}; balance unknown. No addresses printed or funded.",preview.policy,first.index,second.index);
    Ok(())
}
