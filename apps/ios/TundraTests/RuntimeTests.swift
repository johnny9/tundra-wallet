...
final class RuntimeTests: XCTestCase {
    func testPortableBackupAndNativeExportCrossCommonCryptoProvider() throws {
        ...
        var recovered: Tundra? = try Tundra.openProtected(path: restored.path, storageKey: Data(repeating: 0x33, count: 32))
        let restoredWallet = try XCTUnwrap(recovered!.wallets().first)
        XCTAssertFalse(restoredWallet.synced); XCTAssertNil(restoredWallet.totalSats)
        XCTAssertEqual(try recovered!.receiveAddress(walletId: restoredWallet.id).index, 1)
        XCTAssertTrue(try recovered!.exportLabels(walletId: restoredWallet.id).contains("Backup private metadata sentinel"))
        recovered = nil
        XCTAssertEqual(try Data(contentsOf: imported), before)
