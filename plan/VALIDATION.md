...
| Android app | First CI run failed on a generated Kotlin exception `message` field collision; fixed by renaming the Rust record field to `detail`; native rerun pending |
| iOS simulator target | Compiled in GitHub CI for `8fad741`; not run on a simulator or physical phone |
| Kotlin / Swift FFI runtime checks | Three JVM and three macOS Swift smoke tests added for Unicode/reopen, typed errors and integer amounts; CI execution pending |
| QR / USB / bhwi / signing / finalization / broadcast | NOT IMPLEMENTED OR TESTED |
| Git | Baseline and first Rust validation commits pushed to `origin/master` at `johnny9/tundra-wallet` |
